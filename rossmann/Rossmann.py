...
class Rossmann(object):
    ...
    def __init__(self):
        ...
        self.competition_since_year_scaler = pickle.load(
            open(
                self.home_path + 'parameter/competition_since_year_scaler.pkl',
                'rb'))

        self.promo_time_week_scaler = pickle.load(
            open(self.home_path + 'parameter/promo_time_week_scaler.pkl',
                 'rb'))

        self.competition_since_month_scaler = pickle.load(
            open(
                self.home_path +
                'parameter/competition_since_month_scaler.pkl', 'rb'))
    ...
    def feature_engineering(self, df2):
        ...
        df2['competition_since_month'] = (
            (df2['date'] - df2['competition_since']) /
        ...
        df2['promo_since'] = df2['promo_since'].apply(
            lambda x: datetime.strptime(x + '-1', '%Y-%W-%w') - timedelta(days=
                                                                          7))
        ...
        df2['assortment'] = df2['assortment'].apply(
            lambda x: 'basic' if x == 'a' else 'extra'
            if x == 'b' else 'extended')

        df2['state_holiday'] = df2['state_holiday'].apply(
            lambda x: 'public_holiday' if x == 'a' else 'easter_holiday'
            if x == 'b' else 'christmas' if x == 'c' else 'regular_day')

        df2['competition_year'] = ((df2['year'] -
                                    df2['competition_open_since_year']))
    ...
    def data_preparation(self, df5):
        ...
        df5['competition_since_year'] = self.competition_since_year_scaler.transform(
            df5[['competition_year']].values)
        ...
        df5 = pd.get_dummies(df5,
                             ...
                             columns=['state_holiday'])  # One Hot Encoding

        # store_type

        df5['store_type'] = self.store_type_scaler.transform(
            df5['store_type'])
        ...
        df5['assortment'] = df5['assortment'].map(
            assortment_dict)  # Ordinal Encoding
        ...
        cols_selected = [
            ...
            'promo2_since_year', 'competition_since_month', 'promo_time_week',
            'competition_since_year', 'day_of_week_sin', 'day_of_week_cos',
            'week_of_year_sin', 'month_sin', 'month_cos', 'day_sin', 'day_cos',
            'week_of_year_cos'
        ]
